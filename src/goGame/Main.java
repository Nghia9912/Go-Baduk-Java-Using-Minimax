...
public class Main {
	...
	private static class GoGameMain extends JFrame implements ActionListener {
		...
		int boardInt[][] = new int[boardLineCount][boardLineCount];
		...
		ImageIcon downRightBlackIcon = new ImageIcon(direction + "downrightblack.jpg");
		ImageIcon downRightWhiteIcon = new ImageIcon(direction + "downrightwhite.jpg");
		...
		ImageIcon downRightBlackBPointIcon = new ImageIcon(direction + "downrightblack_bPoint.jpg");
		ImageIcon downRightBlackWPointIcon = new ImageIcon(direction + "downrightblack_wPoint.jpg");
		ImageIcon downRightWhiteBPointIcon = new ImageIcon(direction + "downrightwhite_bPoint.jpg");
		ImageIcon downRightWhiteWPointIcon = new ImageIcon(direction + "downrightwhite_wPoint.jpg");
		...
		private void setMenu() {
			...
			String[][] menuStrs = {
					{"GAME", "START", "MINIMAX", "RESULT", "EXIT"}
			};
			String[][] menuCommands = {
					{"GAME", "START", "MINIMAX", "RESULT", "EXIT"}
			};
		...
		public void setOnChessIcon(int i, int j, int userFlag) {
			...
			if (i == 4 && j == 4)
				board[i][j].setIcon(userFlag == 1 ? upLeftBlackIcon : upLeftWhiteIcon);
			else if (i == 4 && j == board[i].length - 5)
				board[i][j].setIcon(userFlag == 1 ? upRightBlackIcon : upRightWhiteIcon);
			else if (i == board.length - 5 && j == 4)
				board[i][j].setIcon(userFlag == 1 ? downLeftBlackIcon : downLeftWhiteIcon);
			else if (i == board.length - 5 && j == board[i].length - 5)
				board[i][j].setIcon(userFlag == 1 ? downRightBlackIcon : downRightWhiteIcon);
			else if (i == 4)
				...
				board[i][j].setIcon(userFlag == 1 ? rightBlackIcon : rightWhiteIcon);
			else if (i == board.length - 5)
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getActionCommand().equals("Exit")) {
				...
				userFlag = 1;
				beginGame();
			} else if (e.getActionCommand().equals("MINIMAX")) {
				userType = false;
				beginGame();
				userFlag = 1;
		...
		private ArrayList checkEat(int i, int j, int userFlag) {
			...
			temp = null;
			int n;
			if (boardInt[a + 1][b] != -userFlag && boardInt[a][b + 1] != -userFlag && boardInt[a - 1][b] != -userFlag
					&& boardInt[a][b - 1] != -userFlag)
				return stonesDead;
			a = i + 1;
		...
		private ArrayList findEatPart(int a, int b, int userFlag) {
			...
			if (countLiberty < 1
					&& !(stonesVisited.size() == 1 && a == stones[steps - 1][0] && b == stones[steps - 1][1] && isKo))
		...
		private Point getComputerPoint() {
			...
			for (i = 4; i < boardInt.length - 4; i++) {
				for (j = 4; j < boardInt[i].length - 4; j++) {
					if (boardInt[i][j] == 0
							&& (checkAvailable(i, j, userFlag) || !checkEat(i, j, userFlag).isEmpty())) {
						stones[steps][0] = i;
						stones[steps][1] = j;
						steps++;
						MinimaxMethod mCM = new MinimaxMethod();
						mCM.setBoard(boardInt, stones, steps, userFlag);
						Point bestMove = mCM.getBestMove(MinimaxMethod.DEPTH, MinimaxMethod.USE_ALPHA_BETA);
						if (bestMove != null) {
							x = bestMove.x;
							y = bestMove.y;
						}
		...
		private void countPoint() {
			...
			int i, j;
			int boardState; // The state of the cross in the board, -1 means white stone, 1 means black
			// stone, 0 means empty.
				...
				for (j = 4; j < board[i].length - 4; j++) {
					...
					if (boardInt[i][j] == 1) {
						...
						if (i == 4 && j == 4)
							...
							board[i][j]
									.setIcon(pointInt[i][j] == 1 ? downLeftBlackBPointIcon : downLeftBlackWPointIcon);
						else if (i == board.length - 5 && j == board[i].length - 5)
							board[i][j]
									.setIcon(pointInt[i][j] == 1 ? downRightBlackBPointIcon : downRightBlackWPointIcon);
						else if (i == 4)
					...
					if (boardInt[i][j] == -1) {
						...
						if (i == 4 && j == 4)
							...
							board[i][j]
									.setIcon(pointInt[i][j] == 1 ? downLeftWhiteBPointIcon : downLeftWhiteWPointIcon);
						else if (i == board.length - 5 && j == board[i].length - 5)
							board[i][j]
									.setIcon(pointInt[i][j] == 1 ? downRightWhiteBPointIcon : downRightWhiteWPointIcon);
						else if (i == 4)
		...
		private ArrayList checkState(int a, int b) {
			...
			int boradState = 2;// -1 means white stone, 1 means black stone, 0 means empty, 2 is the start
			// stage.
			int findCount;
			int begin = 0, end = 0;
			int newx, newy;
			...
			do {
				...
				for (int i = begin; i <= end; i++) {
					...
					if (boardInt[newx + 1][newy] == 0 && !stonesVisited.contains(new Point(newx + 1, newy))) {
						stonesVisited.add(new Point(newx + 1, newy));
						findCount += 1;
					} else if ((boardInt[newx + 1][newy] == 1 || boardInt[newx + 1][newy] == -1)
							&& !stonesVisited.contains(new Point(newx + 1, newy))) {
						if (boradState == 2) {
					...
					if (boardInt[newx][newy + 1] == 0 && !stonesVisited.contains(new Point(newx, newy + 1))) {
						stonesVisited.add(new Point(newx, newy + 1));
						findCount += 1;
					} else if ((boardInt[newx][newy + 1] == 1 || boardInt[newx][newy + 1] == -1)
							&& !stonesVisited.contains(new Point(newx, newy + 1))) {
						if (boradState == 2) {
		...
		private void finish() {
			...
			f.add(new JLabel("DO YOU WANT TO END THE MATCH?"));
